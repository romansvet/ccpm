...
class TestSecurityModel:
    ...
    def test_no_dangerous_wildcard_patterns(self):
        ...
        settings_files = [
            ...
            Path(__file__).parent.parent.parent / "ccpm" / "claude_template" / "settings.local.json",
        ]
        ...
        for settings_file in settings_files:
            if settings_file.exists():
                ...
                for pattern, description in dangerous_patterns.items():
                    ...
                    assert not matches, f"Dangerous pattern found in {settings_file}: {pattern} - {description}"
                ...
                for perm in permissions:
                    if perm.startswith("Bash("):
                        ...
                        if cmd_match:
                            ...
                            assert not command.endswith(" *"), f"Overly broad command: {perm}"
    ...
    def test_directory_access_restrictions(self):
        ...
        for settings_file in settings_files:
            if settings_file.exists():
                ...
                additional_dirs = data.get("permissions", {}).get("additionalDirectories", [])
                for directory in additional_dirs:
                    # Should not allow access to sensitive system directories
                    dangerous_dirs = ["/etc", "/usr", "/var", "/root", "/home", "/Users"]
                    assert not any(directory.startswith(dangerous) for dangerous in dangerous_dirs), \
                        f"Dangerous directory access: {directory}"
                    
                    # Should be scoped to project-related directories
                    assert "/tmp/ccpm" in directory or ".claude" in directory or "ccpm" in directory, \
        ...
        for settings_file in settings_files:
            if settings_file.exists():
                ...
                for pattern in dangerous_file_ops:
                    matches = re.findall(pattern, content, re.IGNORECASE)
                    assert not matches, f"Dangerous file operation found in {settings_file}: {pattern}"
        ...
        for settings_file in settings_files:
            if settings_file.exists():
                ...
                for pattern in dangerous_network_patterns:
                    matches = re.findall(pattern, content, re.IGNORECASE)
                    assert not matches, f"Dangerous network access found in {settings_file}: {pattern}"
        ...
        for settings_file in settings_files:
            if settings_file.exists():
                ...
                for pattern in privilege_escalation_patterns:
                    matches = re.findall(pattern, content, re.IGNORECASE)
                    assert not matches, f"Privilege escalation found in {settings_file}: {pattern}"
        ...
        for settings_file in settings_files:
            if settings_file.exists():
                ...
                for pattern in dangerous_git_patterns:
                    matches = re.findall(pattern, content, re.IGNORECASE)
                    assert not matches, f"Dangerous git operation found in {settings_file}: {pattern}"
        ...
        for settings_file in settings_files:
            if settings_file.exists():
                ...
                for pattern in pip_patterns:
                    ...
                    assert any(pkg in pattern for pkg in [
                        "ccpm", "build", "-e .", "git+https://github.com/"
                    ]) or pattern.endswith("pip install)"), f"Unscoped pip install: {pattern}"
    ...
    def test_no_duplicate_permissions(self):
        ...
        for file_path in permission_files:
            if file_path.exists():
                ...
                assert not duplicates, f"Duplicate permissions found in {file_path}: {duplicates}"
    ...
    def test_no_empty_permissions(self):
        ...
        for file_path in permission_files:
            if file_path.exists():
                ...
                for perm in permissions:
                    assert perm.strip(), f"Empty or whitespace-only permission found in {file_path}"
        ...
        for file_path in permission_files:
            if file_path.exists():
                ...
                if "github.com" in content:
                    assert "automazeio/ccpm" in content, f"Missing main repo reference in {file_path}"
                    
                    # Should not reference other repositories unless explicitly needed
                    unwanted_repos = ["jeremymanning/ccpm"]  # Example of fork that shouldn't be in main config
                    for repo in unwanted_repos:
                        if repo in content:
                            # This might be acceptable in some cases, but flag for review
                            pass
...
class TestSecurityDocumentation:
    ...
    def test_security_documentation_exists(self):
        ...
        for section in required_sections:
            assert section in content, f"Missing security documentation section: {section}"
    ...
    def test_security_documentation_accuracy(self):
        ...
        if security_file.exists():
            ...
            settings_file = Path(__file__).parent.parent.parent / ".claude" / "settings.local.json"
            if settings_file.exists():
                with open(settings_file) as f:
                    data = json.load(f)
                ...
                for perm in sample_permissions:
                    if perm in permissions:
                        ...
                        assert any(keyword in security_content for keyword in [
                            "git", "github", "python", "pytest"
