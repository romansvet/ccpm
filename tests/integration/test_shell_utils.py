"""Integration tests for shell utility functions and cross-platform compatibility."""
...
class TestShellUtilities:
    ...
    @pytest.fixture
    def temp_test_files(self) -> Generator[tuple[Path, Path, Path], None, None]:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            utils_source = Path(__file__).parent.parent.parent / ".claude" / "scripts" / "utils.sh"
            ...
            test_file.write_text("line1: original content\nline2: more content\nline3: final line\n")
    ...
    def _run_bash_with_utils(self, script_content: str, utils_script: Path, cwd: Path = None) -> subprocess.CompletedProcess:
...
"""
    ...
    def test_cross_platform_sed_with_special_characters(self, temp_test_files):
        ...
        test_file.write_text("path/to/file: content\n[bracket]: more\n{brace}: content\n")
...
"""
        ...
        assert original_content in result.stdout  # Should be in backup section
...
"""
    ...
    def test_robust_parse_function(self, temp_test_files):
        ...
        test_file.write_text("""name: test-project
version: 1.0.0
status: active
...
class TestPMScriptIntegration:
    ...
    def pm_test_environment(self, tmp_path: Path) -> Path:
        ...
        # Copy status.sh
        status_source = Path(__file__).parent.parent.parent / ".claude" / "scripts" / "pm" / "status.sh"
    ...
    def test_status_script_with_content(self, pm_test_environment: Path):
        ...
        (prds_dir / "another-feature.md").write_text("# Another Feature\n\nMore content")
...
class TestShellCompatibility:
    ...
    def test_utils_in_different_shells(self, shell: str, tmp_path: Path):
        ...
        try:
            subprocess.run([shell, "--version"], capture_output=True, timeout=5, check=True)
...
class TestEdgeCasesAndErrorScenarios:
    ...
    def test_file_permission_handling(self, tmp_path: Path):
        ...
        # Create read-only file (skip on Windows where this behaves differently)
        ...
        script = f"""#!/bin/bash
source "{utils_script}"
if ! cross_platform_sed 's/readonly/modified/' "{readonly_file}" 2>/dev/null; then
...
"""
        ...
        assert success_count > 0, f"All concurrent operations failed: {[r[2] for r in results]}"
