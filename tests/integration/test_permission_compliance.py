...
''')
    ...
    def test_ccpm_cli_permissions(self):
        ...
        result = subprocess.run(["ccpm", "--help"], capture_output=True, text=True, timeout=10)
        assert result.returncode == 0, f"ccpm --help failed: {result.stderr}"
        
        # Test ccpm without .claude setup (should give appropriate error)
        result = subprocess.run(["ccpm", "list"], capture_output=True, text=True, timeout=10)
...
def test_permission_file_syntax():
    ...
    for file_path in permission_files:
        if file_path.exists():
            ...
            assert all(perm.strip() for perm in permissions), f"Empty permission found in {file_path}"
