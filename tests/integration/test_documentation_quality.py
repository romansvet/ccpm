...
class TestMarkdownQuality:
    ...
    def test_readme_passes_all_lint_rules(self):
        ...
        if result.returncode != 0:
            ...
            for error in errors:
                if error.strip() and not any(acceptable in error for acceptable in acceptable_errors):
                    critical_errors.append(error)
            
            if critical_errors:
                pytest.fail(f"Critical markdownlint errors in README.md:\n" + "\n".join(critical_errors))
    ...
    def test_all_markdown_files_valid(self):
        ...
        for md_file in filtered_files:
            ...
            if result.returncode != 0:
                ...
                if any(critical in errors for critical in ['MD001', 'MD026', 'MD034', 'MD040']):
    ...
    def test_heading_hierarchy_consistent(self):
        ...
        for level_str, title in headings:
            ...
            # No skipping levels
                ...
                pytest.fail(f"Heading level skip from H{prev_level} to H{level}: {title}")
...
class TestYAMLConfiguration:
    ...
    def test_codecov_configuration_valid(self):
        ...
        assert 'file:' not in content.replace('files:', ''), "Codecov should not use deprecated 'file' parameter"
    ...
    def test_pip_caching_enabled(self):
        ...
        setup_python_blocks = re.findall(
            r'uses: actions/setup-python@v\d+.*?(?=- name:|$)', 
            content, 
            re.DOTALL
        )
        ...
        for i, block in enumerate(setup_python_blocks):
            assert "cache: 'pip'" in block, f"Missing pip caching in setup-python block {i+1}: {block[:100]}..."
...
class TestLinkValidation:
    ...
    def test_all_links_accessible(self):
        ...
        if failed_urls:
            error_summary = "\n".join([
                f"{url}: {error}" 
                for url, error in failed_urls[:5]  # Limit output
            ])
...
class TestCLIDocumentationAccuracy:
    ...
    def test_cli_table_matches_actual_commands(self):
        ...
        documented_commands_filtered.discard('--help')  # Remove --help flag from comparison
    ...
    def test_help_command_consistency(self):
        ...
        assert help_flags >= help_commands, "Should prefer 'ccpm --help' over 'ccpm help' for portability"
