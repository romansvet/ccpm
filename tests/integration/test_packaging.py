...
class TestPackagingOperations:
    ...
    def test_fresh_checkout_pip_install(self, clean_temp_env):
        ...
        shutil.copytree(project_root, checkout_path, 
    ...
    def test_fresh_checkout_editable_install(self, clean_temp_env):
        ...
        if len(egg_info_dirs) > 0:
            ...
            if gitignore_path.exists():
                ...
                assert "*.egg-info/" in gitignore_content, "egg-info pattern missing from .gitignore"
    ...
    def test_build_distribution_packages(self, clean_temp_env):
        ...
        ], cwd=checkout_path, capture_output=True, text=True, timeout=300)
    ...
    def test_install_from_wheel(self, clean_temp_env):
        ...
        subprocess.run([str(pip_exe), "install", "build"], 
                      capture_output=True, text=True, timeout=300, check=True)
        
        subprocess.run([str(python_exe), "-m", "build"], 
                      cwd=checkout_path, capture_output=True, text=True, timeout=300, check=True)
        ...
        clean_pip = clean_venv_path / ("Scripts/pip.exe" if os.name == 'nt' else "bin/pip")
        clean_python = clean_venv_path / ("Scripts/python.exe" if os.name == 'nt' else "bin/python")
        ...
        # Test CLI
        ccpm_exe = clean_venv_path / ("Scripts/ccpm.exe" if os.name == 'nt' else "bin/ccpm")
        result = subprocess.run([
            str(ccpm_exe), "--version"
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_multiple_install_uninstall_cycles(self, clean_temp_env):
        ...
        for cycle in range(3):
            ...
            assert result.returncode == 0, f"Install cycle {cycle} failed: {result.stderr}"
            ...
            assert result.returncode == 0, f"Import cycle {cycle} failed: {result.stderr}"
        ...
        dist_dirs = list(checkout_path.glob("dist"))
    ...
    def test_package_metadata_consistency(self, clean_temp_env):
        ...
        assert version_from_setup == "0.1.0", f"Expected '0.1.0', got '{version_from_setup}'"
        ...
        assert 'version = "0.1.0"' in pyproject_content, "Version mismatch in pyproject.toml"
    ...
    def test_dependency_resolution(self, clean_temp_env):
        ...
        for dep in expected_deps:
            ...
            assert result.returncode == 0, f"Dependency {dep} import failed: {result.stderr}"
            assert f"{dep} imported successfully" in result.stdout
        
        # Verify ccpm itself works with its dependencies
        result = subprocess.run([
            str(python_exe), "-c", 
            "import ccpm.cli; import click; import git; import yaml; import requests; print('All imports successful')"
        ], capture_output=True, text=True, timeout=30)
...
class TestRealWorldScenarios:
    ...
    def test_development_workflow(self, clean_temp_env):
        ...
        shutil.copytree(project_root, dev_path, 
                       ignore=shutil.ignore_patterns('__pycache__', '*.pyc', '.git', 
                                                    '.pytest_cache', '*.egg-info'))
