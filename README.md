...
Install CCPM CLI directly from GitHub:
...
```bash
# One-line installer
curl -sSL https://raw.githubusercontent.com/jeremymanning/ccpm/main/installer.sh | bash
```
...
# Or set up in a different directory
ccpm setup /path/to/repo
```
...
```bash
# Project Management
ccpm init          # Initialize PM system (runs /pm:init)
ccpm list          # List all PRDs (runs /pm:prd-list)
ccpm status        # Show project status (runs /pm:prd-status)
ccpm sync          # Sync with GitHub (runs /pm:sync)

# Maintenance
ccpm update        # Update CCPM to latest version
ccpm validate      # Validate system integrity
ccpm clean         # Clean up completed work
ccpm search <term> # Search across all content

# Other
ccpm help          # Show all available commands
ccpm uninstall     # Remove CCPM (preserves pre-existing files)
```

### Working with Existing .claude Directories

If your project already has a `.claude` directory, CCPM will:
1. Back up your existing content
2. Merge it with CCPM's structure
3. Preserve all your customizations
4. Track what was added for clean uninstallation

### Updating CCPM

Keep CCPM up to date with the latest features:
...
# This will:
# - Pull latest changes from automazeio/ccpm
# - Merge updates while preserving your customizations
# - Update the CLI tool itself
```

### Uninstalling

Remove CCPM while preserving any pre-existing `.claude` content:

```bash
ccpm uninstall

# This will:
# - Remove CCPM-specific files
# - Restore any pre-existing .claude content
...
GitHub CLI will be automatically installed if not present.

## Troubleshooting

### GitHub CLI Installation Issues

If automatic installation fails, install GitHub CLI manually:
...
CCPM requires Python 3.8 or higher. Check your version:
```bash
python --version
# or
python3 --version
